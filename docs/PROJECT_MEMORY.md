...
- **2026-09-02 real-phone testing via tunnels** (developer plan, endorsed): repo already pushed to github.com/niranjan323/TellCore (all branches). Used **cloudflared quick tunnels** instead of ngrok (free, no account, no interstitial). **Office network blocks QUIC/UDP → must run `cloudflared tunnel --url ... --protocol http2`**. Two tunnels: API (5106) + web (`vite preview` on 4173; `preview.allowedHosts: true` added to vite.config). CORS: `https://localhost` + `capacitor://localhost` committed in appsettings.json; per-session trycloudflare URLs go in gitignored appsettings.Development.json. APK rebuilt with `VITE_API_BASE_URL=<tunnel>/api/v1 npm run build` + `cap sync` → ~/Desktop/TheUntold-test.apk. **Tunnel URLs change every cloudflared restart → APK must be rebuilt if the API tunnel restarts.** Known limitation: Google blocks OAuth inside webviews → Google sign-in won't work inside the Capacitor app (email/guest work); needs a native plugin (e.g. Capacitor Google Auth / Credential Manager) later — testable on phone BROWSER if the web-tunnel origin is added to Google authorized origins.
- **2026-09-02 phone-QA batch shipped** (developer's 6-point list from real-device testing): ① dashboard horizontal overflow fixed earlier (min-w-0 + overflow-x-clip on AppShell — flex min-width:auto let the stories carousel stretch the page). ② Android hardware back now walks history via @capacitor/app listener in main.tsx (without a listener Capacitor closes the app). ③ "top of story hidden": TWO causes fixed — ScrollToTop on route change (router preserves scroll), and SOTD article needed `relative z-10` (hero's absolute texture layers painted over the static card). ④ Listen = device TTS via hooks/useSpeech.ts (sentence-chunked utterances — long single utterances stall engines; BCP-47 map te→te-IN etc.); reads translated text when active. ⑤ Liked/Saved: StoryFavourites table (script 119), POST /stories/{id}/favourite, GET /stories/liked + /stories/favourites (only still-visible stories: community-published or own), My Stories has My pages/Liked/Saved shelf tabs, SOTD save button wired, StoryResponse gained HasFavourited. ⑥ Translate-on-demand: GET /stories/{id}/translation?lang=xx — checks StoryTranslations cache, else services/ai POST /v1/translate (Groq, faithful, JSON title+text), upserts cache; preview truncation honored; story page has a 13-language picker + "Show original"; free for all users (premium perk unaffected). Edit button also placed in the bottom action stack. Verified live: favourite→shelf, te→en translation. Known nit: translated title sometimes comes back untranslated (model choice; cached).
- **2026-09-02 phone-QA round 2 shipped**: ① Write/pen is now a REGULAR bottom tab (FAB removed from MobileBottomNav — overlapped content; FAB.tsx kept but unused). ② **Android status-bar overlap root cause**: Android WebView reports `env(safe-area-inset-*)` = 0, so CSS pt-safe did nothing in the APK — fixed with `android.adjustMarginsForEdgeToEdge: 'force'` in capacitor.config.ts (Capacitor pads the WebView) + warm-paper `statusBarColor`/`navigationBarColor` + light-bar flags in android styles.xml. iOS/web still use env() (no double padding since env()=0 when margins are forced). ③ **Comments shipped**: StoryComments (script 120), GET/POST `/stories/{id}/comments`, DELETE `/stories/{id}/comments/{commentId}` — delete allowed for comment author OR story owner (CanDelete per-viewer in CommentResponse); same view-access rules as the story; owner notified (new NotificationKind `story-comment`, MessageCircle icon); "Notes from readers" section with composer on StoryDetail (hidden on previews). NO AI moderation on comments yet — noted gap. ④ Fresh APK on Desktop. ⑤ **Listen missing in APK was NOT an old-APK issue**: Android WebView has no `window.speechSynthesis` (web-only API) — fixed with `@capacitor-community/text-to-speech` 8.0.2; useSpeech branches native (plugin, whole-text speak + stop) vs web (chunked utterances). ⑥ Status/navigation bars tinted `#f5f0e8` (surface-secondary "light grey") per developer request — white-on-white blend fixed. Tunnels survive brief disconnects (cloudflared auto-reconnects; URLs unchanged). ⑦ **Android safe areas — final fix**: Capacitor's `adjustMarginsForEdgeToEdge: 'force'` had NO effect on the real device (targetSdk 36 = mandatory edge-to-edge); switched to `@capawesome/capacitor-android-edge-to-edge-support` 8.0.8 (EdgeToEdge.enable() + setBackgroundColor('#f5f0e8') in main.tsx, android-only dynamic import) with the built-in option set to 'disable' to avoid double padding. CSS pt-safe still covers iOS/web (env()=0 on Android so no conflict).
